...
class DiffusionEquation(Equation):
    ...
    def loop(self, d_idx, s_idx, d_c, d_ac, s_c, s_m, d_rho, s_rho, d_D, s_D, XIJ, R2IJ, DWIJ, WIJ, EPS):
        ...
        rijdotdwij = XIJ[0]*DWIJ[0] + XIJ[1]*DWIJ[1]
        ...
        d_ac[d_idx] += mj*Dij*(rhoi+rhoj)*visc*cij/(rhoi*rhoj) + 0.0*mj/rhoj*cj*(1-cj)*WIJ

      

class DKDSPHStep(IntegratorStep):
    def initialize(self, d_idx, d_c, d_co):
